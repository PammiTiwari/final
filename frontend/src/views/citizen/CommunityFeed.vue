<template>
  <div class="app-layout">
    ...
    <div class="main-content">
      <AppTopbar title="Community Feed" />
          ...
          <div v-for="post in filteredPosts" :key="post.id" class="post-card card">
            ...
            <div v-if="expandedPost === post.id" class="comments-section">
              ...
              <div class="comment-input">
                <input v-model="commentTexts[post.id]" class="form-control" placeholder="Write a comment..." :disabled="postingComment[post.id]" @keydown.enter="addComment(post)" />
                <button class="btn btn-sm btn-primary" :disabled="postingComment[post.id]" @click="addComment(post)">Post</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <div v-if="showCreatePost" class="modal-overlay" @click.self="closeCreate">
      <div class="modal create-modal">
        <div class="create-header">
          ...
          <button class="create-action" :disabled="!canShare || postLoading" @click="createPost">
        ...
        <div class="create-body">
          <div class="form-group">
            <label>Caption</label>
            <textarea v-model="newPost.content" class="form-control" rows="4" placeholder="Write a caption…"></textarea>
          </div>
          <div class="form-group">
...
const canShare = computed(() => !!(newPost.value.content.trim() || newPost.value.image_urls.length))
...
<style scoped>
.posts-list { display: flex; flex-direction: column; gap: 1rem; max-width: 680px; }
.post-card { padding: 1.1rem 1.25rem; }
.post-header { display: flex; justify-content: space-between; align-items: flex-start; gap: 0.75rem; margin-bottom: 0.75rem; }
...
.post-title { font-size: 0.95rem; font-weight: 700; color: #5C1A41; margin-bottom: 0.4rem; }
...
.action-btn { background: none; border: none; font-size: 0.8rem; color: #B0708F; cursor: pointer; display: flex; align-items: center; gap: 0.25rem; padding: 0.2rem 0.4rem; border-radius: 5px; transition: 0.12s; }
.action-btn:hover { background: #FFE9F2; color: #5C1A41; }
...
.comment-input .form-control { font-size: 0.8rem; }
